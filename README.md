...
- `manager.db`：订阅、用量快照、订阅规则、节点、监听、规则集、分流设置、保存版本和应用状态，SQLite WAL 模式。
- `config.yaml`：内核运行配置，由管理服务维护。
- `last-good.yaml`：上一次应用并验证成功的配置。
- `core.secret`：管理服务调用内核的独立认证口令。
- `ruleset/`：内核下载的规则集缓存，可以删除，下次会重新拉取。

旧数据库启动时自动升级：`listeners` 表补上出口方式并允许规则监听不绑定节点，同时补建规则集和分流设置表并写入默认值。已有监听保持原端口、原绑定和原行为。删除内置规则集后重启不会重新写回。
...
生成六个策略组：节点选择、自动选择、国内直连、国外代理、广告拦截、漏网之鱼。每个组都保留 DIRECT 成员，因此没有可用节点时配置依然能通过校验。

规则自上而下匹配，第一条命中即生效，顺序为：

1. 私有网段字面规则（回环、内网、CGNAT、链路本地）直连，不触发 DNS 解析。
2. 按排序值排列的规则集，默认是私有域名/IP（100、110）、广告拦截（200）。
3. 订阅自带的规则，默认插入在排序值 500 的位置，即广告拦截之后、国内外规则集之前。服务类规则因此优先于宽泛的国内外划分。
4. 其余规则集，默认是国外域名（600）、国内域名（700）、国内 IP（710）。
5. 兜底 `MATCH`，指向漏网之鱼。
...
开启后生成配套 DNS：默认使用国内解析器，出境规则集匹配到的域名使用国外解析器，节点自身的域名用 `proxy-server-nameserver` 单独解析以避免首次连接时的循环依赖。
