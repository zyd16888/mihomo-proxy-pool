...
- `manager.db`：订阅、用量快照、订阅规则、节点、监听、规则集、代理组及其选择、分流设置、保存版本和应用状态，SQLite WAL 模式。
...
节点停用或被订阅移除后，不再作为可用成员输出；原选择失效时页面提示重新选择，手动组使用剩余默认成员。自动组没有可用成员时拦截连接。手动组还可显式选择默认节点组、DIRECT 或 REJECT。自建组被规则集或兜底策略引用时不能删除；名称在创建后保持稳定。订阅提供的目标组消失后，自定义规则集和失效兜底使用 REJECT，并提示更换出口。
...
- **策略组成员**：订阅里的节点名会映射到生成配置中的节点。`filter`、`include-all`、`use` 在生成配置时就地展开成具体成员列表——生成配置把所有节点改名为 `node-<id>`，按名字过滤在内核里已经匹配不到任何东西。解析后没有可用成员的策略组会被丢弃，指向它的规则一并丢弃；这会连锁进行直到稳定。
