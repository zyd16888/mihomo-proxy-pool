...
const pages = {
  ...
  nodes: {title:'节点池', kicker:'NODE LIBRARY', description:'管理可选节点，按需绑定到监听端口。', create:'＋ 导入节点', table:'全部节点', note:'节点本身不开放本地端口', foot:'停用节点会关闭其绑定监听。被监听引用的节点不能直接删除。'},
...
function renderTable() {
  ...
  const items=state[page].filter(item=>[item.name,item.port,item.server,activeNode(item.nodeId)?.name].some(v=>String(v??'').toLowerCase().includes(term)));
  $('row-count').textContent=`${items.length} 条记录${term?` / 共 ${state[page].length} 条`:''}`;
  if (!items.length) {const empty=term?'没有匹配的记录':{listeners:'还没有监听端口',nodes:'节点池为空',subscriptions:'还没有订阅'}[page];const hint=term?'试试其他关键词。':{listeners:'先导入节点，再创建你的第一条固定线路。',nodes:'导入节点链接，或添加订阅获取节点。',subscriptions:'添加订阅地址，随后同步节点。'}[page];$('table').innerHTML=`<div class="empty"><strong>${empty}</strong><p>${hint}</p></div>`;return;}
  let headers,rows;
  if (page==='listeners') {
    ...
    headers=['节点名称','远程地址','来源','状态','操作'];
    rows=items.map(n=>`<tr><td class="primary-cell">${escapeHTML(n.name)}<span class="cell-sub">${escapeHTML(n.protocol.toUpperCase())} · ${state.listeners.filter(l=>l.nodeId===n.id).length} 个绑定</span></td><td class="mono">${escapeHTML(n.server)}:${n.port}</td><td>${escapeHTML(nodeSource(n))}</td><td>${!n.available?tag('订阅已移除','error'):n.enabled?tag('可绑定','ready'):tag('已停用')}</td><td><div class="actions">${button('检测','check',n.id)}${button(n.enabled?'停用':'启用','toggle-node',n.id)}${button('删除','delete-node',n.id,'remove')}</div></td></tr>`);
  } else {
...
$('table').addEventListener('click',e=>{
  ...
  run(async()=>{
    ...
    if(action==='check'){const data=await api('/api/nodes/'+id+'/check','POST',{});notice(`${activeNode(id).name}：${data.delay} ms`);}
